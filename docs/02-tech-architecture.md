...
│   ├── ObjectPool.gd
│   └── RoundController.gd       # 라운드 시작/종료, 아티팩트 지급 판정
│
├── tests/                       # GUT
...
### 9.2 ★ 발광부 틴트 (무채색 전면 틴트를 대체)

**이 프로젝트에서 가장 큰 절약 지점입니다.**

타워는 족보 10종 × 속성 4종 = **40가지 조합**입니다. 전부 그리면 AI 일관성이 반드시 무너집니다.

**당초 계획은 "무채색으로 그리고 전체를 틴트"였으나 폐기했습니다.**
채택한 아이소메트릭 픽셀아트 스타일과 양립하지 않기 때문입니다 —
석재의 갈색, 이끼의 녹색, 흙의 황토색이 전부 속성색으로 물들면 그 느낌이 사라집니다.
돌은 돌색이어야 합니다.
...
| 부위 | 처리 |
|---|---|
| 석재 · 흙 · 풀 · 나무 · 소품 | **고정 색.** 틴트하지 않음 |
| 창문 · 룬 · 수정 · 마법 이펙트 | **밝은 흰빛으로 생성** → 셰이더가 속성색으로 틴트 |

```gdscript
// 휘도 임계값으로 발광부만 골라내 속성색을 입힌다
...
**절감 효과는 그대로 40장 → 10장입니다.** 오히려 결과물이 더 좋습니다.

> **생성 단계에서 반드시 지킬 것:** 창문·룬을 보라색 등 특정 색으로 뽑으면
> 틴트가 그 위에 덧칠되어 4속성이 전부 탁해집니다. **반드시 밝은 흰빛으로 생성하세요.**
> 상세는 [assets/_prompts/00-style-sheet.md](../assets/_prompts/00-style-sheet.md) 2절.

같은 기법을 몬스터에도 적용합니다 (유형 7종 → 7장).
...
4번의 다운스케일 방식이 중요합니다. **픽셀아트는 반드시 nearest neighbor로 축소**하세요.
lanczos나 bilinear를 쓰면 픽셀 격자가 뭉개져 픽셀아트가 아니게 됩니다.
**축소 후 반드시 실제 타일 크기로 확인**하세요.
